...
def do_deploy(archive_path):
    ...
    run("tar -xzf /tmp/{} -C /data/web_static/releases/{}".fotmat(archive, archive[0:-4]))
    run("mv /data/web_static/releases/{}/web_static/*\
            /data/web_static/releases/{}/".format(archive[0:-4], archive[0:-4]))
    run("rm -rf /tmp/{}".format(archive))
    run("rm -f /data/web_static/releases/{}/web_static".fotmat(archive))
    run("rm /data/web_static/current")
    run("ln -s /data/web_static/releases/{}/ /data/web_static/current".format(archive[0:-4]))
